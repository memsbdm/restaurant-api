...
type UserRepository interface {
	GetByID(ctx context.Context, id uuid.UUID) (codegen.User, error)
	...
	Update(ctx context.Context, userDto *dto.UserDTO) (codegen.User, error)
...
func (r *userRepository) GetByID(ctx context.Context, id uuid.UUID) (codegen.User, error) {
	user, err := r.queries.GetUserById(ctx, id)
	if err != nil {
		log.Printf("error: GetByID (user repository): %v", err)
...
func (r *userRepository) Update(ctx context.Context, userDto *dto.UserDTO) (codegen.User, error) {
	updatedUser, err := r.queries.UpdateUser(ctx, userDto.ToUpdateParams())
	if err != nil {
		log.Printf("error: Update (user repository): %v", err)
		return codegen.User{}, err
	}
